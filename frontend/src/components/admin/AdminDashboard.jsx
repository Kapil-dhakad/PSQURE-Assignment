...
const AdminDashboard = () => {
  ...
  const fetchTrips = async () => {
    try {
      const response = await fetch("http://localhost:3000/api/v1/trips/admin", {
        method: "GET",
        credentials: "include",
        headers: {
      ...
      if (!response.ok) {
        throw new Error(`Trips API error! status: ${response.status}`);
      }
    ...
    const fetchData = async () => {
      try {
        ...
        const [tripsResponse, bookingsResponse] = await Promise.all([
          fetch("http://localhost:3000/api/v1/trips/admin", {
            method: "GET",
            credentials: "include",
            headers: {
              "Content-Type": "application/json",
          ...
          fetch("http://localhost:3000/api/v1/bookings/admin/bookings", {
            method: "GET",
            credentials: "include",
            headers: {
              "Content-Type": "application/json",
        ...
        if (!tripsResponse.ok) {
          throw new Error(`Trips API error! status: ${tripsResponse.status}`);
        }
        if (!bookingsResponse.ok) {
          throw new Error(`Bookings API error! status: ${bookingsResponse.status}`);
        }
  ...
  const handleDeleteTrip = async (tripId) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/api/v1/trips/delete/${tripId}`,
        {
          method: "DELETE",
          credentials: "include",
          headers: {
